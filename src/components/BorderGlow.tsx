...
export default function BorderGlow({
  ...
  const getEdgeProximity = useCallback(
    (element: HTMLElement, x: number, y: number) => {
      ...
      let factorX = Number.POSITIVE_INFINITY;
      let factorY = Number.POSITIVE_INFINITY;

      if (deltaX !== 0) factorX = centerX / Math.abs(deltaX);
      if (deltaY !== 0) factorY = centerY / Math.abs(deltaY);

      return Math.min(Math.max(1 / Math.min(factorX, factorY), 0), 1);
    },
    [getCenterOfElement]
  );
